...
export async function generateRooms(jobData: RoomGenerationJobSchema) {
    ...
    logger.info(`Generating rooms for room category ${roomCategory.id} from ${startDate.toISOString()} to ${endDate.toISOString()} (${totalDays} days)`);
...
export async function processDateBatch(roomCategory: RoomCategory, startDate: Date, endDate: Date, priceOverride?: number) {
    ...
    // TODO: Optimize this to reduce database calls
    while (currentDate <= endDate) {
    ...
    if(roomsToCreate.length > 0) {
        logger.info(`Creating ${roomsToCreate.length} rooms for room category ${roomCategory.id}`);
